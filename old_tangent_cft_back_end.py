...
from Embedding_Preprocessing.encoder_tuple_level import (
    TupleEncoder,
    TupleTokenizationMode,
)

from old_tangent_cft_module import TangentCFTModule
...
class TangentCFTBackEnd:
    def __init__(
        ...
        if is_wiki:
            self.data_reader = WikiDataReader(
                path_data_set, read_slt, queries_directory_path
            )
        else:
    ...
    def train_model(
        ...
        # Verificar se existem dados pré-processados para carregar
        if use_preprocessed and preprocessed_file and os.path.isfile(preprocessed_file):
            ...
            # Salvar os dados pré-processados se um arquivo for especificado
            if preprocessed_file:
                self.__save_preprocessed_data(
                    dictionary_formula_tuples_collection, preprocessed_file
                )

        self.__save_encoder_map(map_file_path)
        print("training the fast text model...")
        self.module.train_model(
            self.config, list(dictionary_formula_tuples_collection.values())
        )
    ...
    def retrieval(
        ...
        tensor_values, index_formula_id = self.module.index_collection_to_tensors(
            dictionary_formula_tuples_collection
        )
        ...
        for query in dictionary_query_tuples:
            encoded_tuple_query = self.__encode_lst_tuples(
                dictionary_query_tuples[query],
                embedding_type,
                ignore_full_relative_path,
                tokenize_all,
                tokenize_number,
            ...
            retrieval_result[query] = self.module.formula_retrieval(
                tensor_values, index_formula_id, query_vec
            )
        # print(len(retrieval_result))
        return retrieval_result

    def get_collection_query_vectors(
        self,
        dictionary_formula_tuples_collection,
        embedding_type,
        ignore_full_relative_path,
        ...
        """
        index_formula_id = self.module.index_collection_to_numpy(
            dictionary_formula_tuples_collection
        )
        dictionary_query_tuples = self.data_reader.get_query()
        query_vectors = {}
    ...
    def __encode_train_tuples(
        self, embedding_type, ignore_full_relative_path, tokenize_all, tokenize_number
    ):
        """
        ...
        for formula in dictionary_formula_slt_tuple:
            dictionary_lst_encoded_tuples[formula] = self.__encode_lst_tuples(
    ...
    def __save_encoder_map(self, map_file_path):
        ...
        for item in self.encoder_map_node:
            file.write(
                "N" + "\t" + str(item) + "\t" + str(self.encoder_map_node[item]) + "\n"
            )
        for item in self.encoder_map_edge:
            file.write(
                "E" + "\t" + str(item) + "\t" + str(self.encoder_map_edge[item]) + "\n"
            )
        file.close()
    ...
    def __save_preprocessed_data(self, data, file_path):
        """
        Salva os dados pré-processados em um arquivo usando pickle.
        """
        print(f"Salvando dados pré-processados em {file_path}...")
        with open(file_path, "wb") as f:
            pickle.dump(data, f)
        print("Dados pré-processados salvos com sucesso!")

    def __load_preprocessed_data(self, file_path):
        """
        Carrega os dados pré-processados de um arquivo pickle.
        """
        print(f"Carregando dados pré-processados de {file_path}...")
