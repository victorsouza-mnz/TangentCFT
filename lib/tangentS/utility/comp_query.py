...
from lib.tangentS.ranking.query import Query
from lib.tangentS.utility.text_query import TQuery
from lib.tangentS.math_tan.symbol_tree import SymbolTree
from lib.tangentS.math_tan.layout_symbol import LayoutSymbol

# from lib.tangentS.ranking.ranking_functions import similarity_v06, similarity_v09, similarity_v10, similarity_v11
from lib.tangentS.ranking.reranker import Reranker
...
class CompQueryResult:  # results for a single document
    ...
    def add_mscore(self, qexpr, candidate):
        match = MathScore(
            qexpr, candidate
        )  # N.B. qexpr will repeat if more than one match in a document
...
class CompQuery:
    ...
    def pivot_by_docs(self, how):
        ...
        if self.mqueries:
            ...
            raise Exception(
                "Re-ranking math_tan expressions on combined queries not implemented"
            )
                ...
                for result in query.results.values():
                    ...
                    if how == "MSS":  # compute the MSS score if requested
                        ...
                        result.new_scores = sim_res[
                            0
                        ]  # scores returned as first component of result -- other components are node sets
                    elif how == "v09":
    ...
    def combine_math(self, how, msize_norm):
        ...
        if self.by_document is None:
            self.pivot_by_docs(
                how
            )  # must pivot by documents before combining; compute MSS if need be
        ...
        if size_based_normalization:
            ...
            for i, q in enumerate(self.mqueries):
                qlens.append(
                    q.expression.count("[")
                )  # number of nodes is proxy for number of tuples
                alllength += qlens[i]
            ...
            if not doc.mscores:  # no math_tan formula scores for the document
                ...
                for mathscore in doc.mscores:
                    ...
                    try:
                        ...
                        if (
                            mathscore.candidate.new_scores
                            > oldscore.candidate.new_scores
                        ):
    ...
    def combine_math_text(self, rerank, mweight, mdynamic, msize_norm, mtext_only):
        ...
        for doc in self.msorted_docs:
            ...
            doc.final_score = [
                mweight * float(mscore)
                + tweight * float(doc.tscore[1] if self.tquery.keywords else 1.0)
            ]  # use normalized text score
        ...
        for expr in doc.mscores:
            if expr.top_in_doc:
                for loc in expr.candidate.locations:
                    if (
                        loc[0] == doc.doc_id
                    ):  # all doc offsets for the identical formula are listed
                        pos.append(loc[1])
        return pos
    ...
    def output_query(self, out_file, cntl, topk, query_time_ms):
        ...
        if len(self.sorted_docs) < topk:
            print(
                "Warning: Query produced less than "
                + str(topk)
        ...
        for idx in range(topk):
            ...
            try:
                exprids = list(
                    map(
                        lambda pos: (d.find_mathml_id(doc.doc_id, pos[0]), pos[1]),
                        positions,
                    )
                )
