...
def get_vectors(
    ...
    index_formula_id, query_vectors = system.get_collection_query_vectors(
...
def retrieval(collection_vectors, index_formula_id, query_vectors):
    ...
    for query in query_vectors:
        ...
        retrieval_result[query] = TangentCFTModule.formula_retrieval(
            collection_vectors, index_formula_id, query_vec
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="This is used to combine vector (by summation with SLT, OPT, and SLT Type."
    )
    ...
    parser.add_argument(
        "-qd", type=str, help="NTCIR12 query directory.", default="TestQueries/"
    )
    ...
    index_formula_id_slt, query_vectors_slt = get_vectors(
        dataset_file_path,
        queries_directory_path,
        is_wiki,
        config_id=1,
    ...
    index_formula_id_opt, query_vectors_opt = get_vectors(
        dataset_file_path,
        queries_directory_path,
        is_wiki,
        config_id=2,
    ...
    index_formula_id_slt_type, query_vectors_slt_type = get_vectors(
        dataset_file_path,
        queries_directory_path,
        is_wiki,
        config_id=3,
    ...
    tensor_values, index_formula_id = sum_collection(
        index_formula_id_slt, index_formula_id_opt, index_formula_id_slt_type
    )
    query_vectors = sum_queries(
        query_vectors_slt, query_vectors_opt, query_vectors_slt_type
    )
