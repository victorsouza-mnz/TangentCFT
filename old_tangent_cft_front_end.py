...
from old_tangent_cft_back_end import TangentCFTBackEnd


def main():
    parser = argparse.ArgumentParser(
        description="Given the configuration file for training Tangent_CFT model."
    ...
    parser.add_argument(
        "--t",
        type=bool,
        help="Value True for training a new model and False for loading a model",
        default=True,
    )
    parser.add_argument(
        "--r",
    ...
    parser.add_argument(
        ...
        "should be csv file of formula",
        required=True,
    )
    parser.add_argument(
        "-cid", metavar="cid", type=int, help="Configuration file.", required=True
    )
    parser.add_argument(
        "--wiki",
        type=bool,
    ...
    parser.add_argument(
        "--slt",
        type=bool,
        help="Determines to use slt (True) or opt(False)",
        default=True,
    )
    parser.add_argument(
        "-em", type=str, help="File path for encoder map.", required=True
    )
    ...
    parser.add_argument(
        "--rf", type=str, help="Retrieval result file path.", default="ret_res"
    )
    parser.add_argument("--ri", type=int, help="Run Id for Retrieval.", default=1)
    parser.add_argument(
        "--frp", type=bool, help="Determines to ignore full relative path", default=True
    )
    parser.add_argument(
        "--ta", type=bool, help="Determines to tokenize all", default=False
    )
    parser.add_argument(
        "--tn", type=bool, help="Determines to tokenize numbers", default=True
    )
    parser.add_argument(
        "--et",
        help="Embedding type; 1:Value, 2:Type, 3:Type and Value separated and"
        " 4: Type and Value Not Separated",
        choices=range(1, 5),
        default=3,
        type=int,
    )
    parser.add_argument(
        "--use_preprocessed",
        action="store_true",
        help="Usar dados pré-processados em vez de processar novamente",
    )
    parser.add_argument(
        "--preprocessed_file",
        type=str,
        default=None,
        help="Caminho para o arquivo com dados pré-processados",
    )
    ...
    system = TangentCFTBackEnd(
        ...
        path_data_set=dataset_file_path,
        is_wiki=is_wiki,
        read_slt=read_slt,
    ...
    if train_model:
        ...
        if do_retrieval:
            ...
            system.create_result_file(
                retrieval_result, "Retrieval_Results/" + res_file, run_id
            )
    else:
